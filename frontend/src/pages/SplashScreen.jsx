...
const getAssignment = () => {
  ...
  const initialTheme = Math.random() < 0.5 ? "light" : "dark";
  const assignment = {
    variant, initialTheme, currentTheme: initialTheme,
    experimentId: `EXP-${String(Math.floor(1000 + Math.random() * 9000))}`,
    confidence: 98.4,
  };
  localStorage.setItem("experimentAssignment", JSON.stringify(assignment));
...
// ── Section wrapper — alternates background to create visual separation ──
...
// ── Stat card with bigger numbers ────────────────────────────────────────
function StatItem({ value, label, isDark }) {
  return (
    <div className="text-center px-4">
      <p className={`text-3xl md:text-4xl font-display font-black ${isDark ? "text-white" : "text-gray-900"}`}>
        {value}
      </p>
      <p className={`text-sm mt-1.5 font-medium ${isDark ? "text-white/40" : "text-gray-500"}`}>{label}</p>
    </div>
  );
}
...
export default function SplashScreen() {
  ...
  const [copied, setCopied] = useState(false);
  const [stats, setStats] = useState({
    timeOnPage: "2m 54s", scrollDepth: 94, interactions: 18, pagesViewed: 1,
  });
  ...
  // Shared card class — strong definition in both themes
  const cardCls = `rounded-2xl border p-7 transition-colors duration-300 ${
    isDark
      ? "bg-white/[0.04] border-white/[0.1] hover:border-white/[0.18]"
      : "bg-white border-gray-200 shadow-[0_2px_16px_rgba(0,0,0,0.07)] hover:shadow-[0_4px_24px_rgba(108,92,231,0.12)]"
  }`;

  // Shared CTA style
  const ctaPrimary = `inline-block px-8 py-3.5 rounded-xl text-white font-semibold
    bg-gradient-to-r from-brand-violet to-brand-blue
    shadow-[0_0_30px_rgba(108,92,231,0.4)]
    hover:shadow-[0_0_45px_rgba(108,92,231,0.6)]
    hover:scale-[1.02] active:scale-[0.99]
    transition-all duration-200`;

  return (
    <div className={`min-h-screen transition-colors duration-500 ${isDark ? "bg-[#09090B]" : "bg-white"}`}>

      {/* ── Avatar particle field — clipped to hero viewport only ── */}
      <div className="absolute top-0 left-0 right-0 h-screen pointer-events-none overflow-hidden z-0">
        <AvatarParticleField />
      </div>

      {/* ── Ambient glow — only in hero area, clipped ── */}
      <div className="absolute top-0 left-0 right-0 h-screen pointer-events-none overflow-hidden z-0">
        <div className={`absolute top-[-10%] left-[10%] w-[500px] h-[500px] rounded-full blur-[130px] ${
          isDark ? "bg-brand-violet/20" : "bg-brand-violet/08"
        }`} />
        <div className={`absolute top-[20%] right-[-5%] w-[400px] h-[400px] rounded-full blur-[130px] ${
          isDark ? "bg-brand-blue/18" : "bg-brand-blue/06"
        }`} />
      </div>
      ...
      <div className="relative z-10">
          ...
          <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-14 items-center">
            ...
            >
              <span className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full
                               bg-brand-violet/10 text-brand-violet border border-brand-violet/20 mb-6">
                <motion.span
              ...
              <h1 className={`text-5xl md:text-6xl font-display font-extrabold leading-[1.06] tracking-tight ${
                isDark ? "text-white" : "text-gray-900"
              }`}>
                Measure.
                <br />Experiment.
                <br />
                <span className="bg-gradient-to-r from-brand-violet via-fuchsia-500 to-brand-blue bg-clip-text text-transparent">
                  Optimize.
                </span>
              </h1>

              <p className={`mt-6 text-lg leading-relaxed max-w-md ${isDark ? "text-white/50" : "text-gray-500"}`}>
                Run A/B tests with confidence. Get AI-powered insights and grow your business faster.
              </p>
              ...
              <div className="mt-8 flex flex-wrap items-center gap-3">
                ...
                <button className={`px-6 py-3.5 rounded-xl font-medium border transition-all duration-200 ${
                  isDark
                    ? "border-white/12 text-white/65 hover:text-white hover:border-white/25 hover:bg-white/[0.04]"
              ...
              <div className="mt-6 flex flex-wrap gap-5">
                {["No credit card", "14-day free trial", "Cancel anytime"].map((t) => (
                  <span key={t} className={`flex items-center gap-1.5 text-sm ${isDark ? "text-white/30" : "text-gray-400"}`}>
        ...
        {/* ════════════════ STATS ════════════════ */}
        <Section tinted>
          <div className={`border-y py-12 ${isDark ? "border-white/[0.06]" : "border-gray-100"}`}>
            <div className="max-w-4xl mx-auto px-6">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-0 md:divide-x divide-gray-200 dark:divide-white/[0.06]">
                {[
                  { value: "10K+", label: "Experiments run" },
                  { value: "98.7%", label: "Avg. confidence" },
                  { value: "+14.2%", label: "Avg. lift detected" },
                  { value: "500K+", label: "Visitors/month" },
                ].map((s) => <StatItem key={s.label} {...s} isDark={isDark} />)}
              </div>
            </div>
          </div>
        </Section>
        ...
        <Section>
          <div className="py-10 px-6 max-w-5xl mx-auto text-center">
            <p className={`text-xs font-semibold tracking-widest uppercase mb-7 ${isDark ? "text-white/25" : "text-gray-400"}`}>
            ...
            <p className={`text-[10px] mt-5 ${isDark ? "text-white/15" : "text-gray-300"}`}>
        ...
        <Section className="py-20 px-6">
          <div className="absolute inset-0 pointer-events-none overflow-hidden">
            <div className={`absolute left-1/2 top-1/2 h-[500px] w-[500px] -translate-x-1/2 -translate-y-1/2 rounded-full blur-[140px] ${
              isDark ? "bg-brand-violet/12" : "bg-brand-violet/05"
            }`} />
          </div>
          ...
          <div className="relative max-w-5xl mx-auto">
            <motion.div
              ...
              className="text-center mb-12"
            >
              <div className={`inline-flex items-center gap-2.5 px-4 py-1.5 rounded-full border text-xs font-semibold mb-5 ${
                isDark
                  ? "bg-brand-violet/10 border-brand-violet/20 text-brand-violet"
                  : "bg-brand-violet/8 border-brand-violet/20 text-brand-violet"
              }`}>
                Experiment ID
                <span className="font-mono">{assignment.experimentId}</span>
              </div>
              ...
              <p className={`mt-4 text-lg ${isDark ? "text-white/40" : "text-gray-500"}`}>
                Thank you for helping improve ExperimentX.
              </p>
            </motion.div>

            {/* 3 report cards — more padding, bigger text */}
            <div className="grid md:grid-cols-3 gap-5">
              ...
              <motion.div
                ...
                className={cardCls}
              >
                <h3 className={`text-xs font-bold uppercase tracking-widest flex items-center gap-2 mb-5 ${
                ...
                <div className="space-y-5">
                  <div>
                    <p className={`text-xs mb-1 ${isDark ? "text-white/35" : "text-gray-400"}`}>Hero Variant</p>
                    <p className="text-3xl font-display font-bold text-brand-violet">Variant {heroVariant}</p>
                  </div>
                  <div>
                    <p className={`text-xs mb-1 ${isDark ? "text-white/35" : "text-gray-400"}`}>Theme Journey</p>
                    <div className="flex items-center gap-2">
                      <span className={`text-sm font-medium ${isDark ? "text-white/70" : "text-gray-700"}`}>
                        {initialThemeEmoji} {initialThemeLabel}
                      </span>
                      {themeSwitched && (
                        <>
                          <span className={isDark ? "text-white/25" : "text-gray-300"}>→</span>
                          <span className={`text-sm font-medium ${isDark ? "text-white/70" : "text-gray-700"}`}>
                            {themeEmoji} {currentThemeLabel}
                          </span>
                        </>
                      )}
                    </div>
                    {themeSwitched && (
                      <span className="inline-block mt-2 text-xs font-semibold text-emerald-500 bg-emerald-500/10 px-2.5 py-1 rounded-full">
                        Theme changed ✓
                      </span>
                    )}
                  </div>
                  <div>
                    <p className={`text-xs mb-2 ${isDark ? "text-white/35" : "text-gray-400"}`}>Confidence</p>
                    <div className="flex items-center gap-3">
                      <div className={`flex-1 h-2 rounded-full overflow-hidden ${isDark ? "bg-white/10" : "bg-gray-100"}`}>
                        <motion.div
                          className="h-full bg-gradient-to-r from-brand-violet to-brand-blue rounded-full"
                          initial={{ width: 0 }}
                          whileInView={{ width: `${assignment.confidence}%` }}
                          viewport={{ once: true }}
                          transition={{ duration: 1, ease: "easeOut" }}
                      ...
                      <span className={`text-sm font-mono font-bold ${isDark ? "text-white/70" : "text-gray-700"}`}>
              ...
              >
                ...
                <div className="space-y-4">
                  {[
                    { label: "Time on page", value: stats.timeOnPage, pct: 100, color: "bg-brand-violet" },
                    { label: "Scroll depth", value: `${stats.scrollDepth}%`, pct: stats.scrollDepth, color: "bg-brand-blue" },
                    { label: "Interactions", value: stats.interactions, pct: Math.min(100, (stats.interactions / 20) * 100), color: "bg-emerald-500" },
                    { label: "Pages viewed", value: stats.pagesViewed, pct: Math.min(100, stats.pagesViewed * 30), color: "bg-amber-500" },
                  ].map((s) => (
                    <div key={s.label}>
                      <div className="flex justify-between mb-1.5">
                        <span className={`text-sm ${isDark ? "text-white/45" : "text-gray-500"}`}>{s.label}</span>
                        <span className={`text-sm font-mono font-semibold ${isDark ? "text-white/70" : "text-gray-700"}`}>{s.value}</span>
                      </div>
              ...
              <motion.div
                ...
                className={`${cardCls} ${
                  isDark
                    ? "bg-gradient-to-br from-brand-violet/8 to-brand-blue/5 border-brand-violet/20"
                    : "bg-gradient-to-br from-brand-violet/5 to-brand-blue/3 border-brand-violet/20"
                }`}
              >
                <h3 className="text-xs font-bold uppercase tracking-widest flex items-center gap-2 mb-5 text-brand-violet">
                  <Brain size={14} />
                  AI Observations
                </h3>
                <ul className="space-y-3">
                  {[
                    `Engaged with Variant ${heroVariant} hero`,
                    themeSwitched ? "Switched to dark theme" : "Maintained initial theme",
                    "Read comparison section",
                    "Viewed feature cards",
                    "High engagement — likely to convert",
                  ].map((obs, i) => (
                    <li key={i} className="flex items-start gap-2.5">
                      <CheckCircle2 size={15} className="text-brand-violet flex-shrink-0 mt-0.5" />
                      <span className={`text-sm leading-snug ${isDark ? "text-white/65" : "text-gray-600"}`}>{obs}</span>
                    </li>
                  ))}
                </ul>
              </motion.div>
            ...
            >
              <div className={`inline-flex items-center gap-2.5 px-5 py-2.5 rounded-full border mb-8 ${
                isDark
                  ? "bg-brand-violet/10 border-brand-violet/20"
                  : "bg-brand-violet/8 border-brand-violet/20"
              }`}>
              ...
              <p className={`text-xl leading-relaxed max-w-xl mx-auto mb-8 ${isDark ? "text-white/55" : "text-gray-500"}`}>
                Every visitor teaches your product something.{" "}
                <span className="bg-gradient-to-r from-brand-violet to-brand-blue bg-clip-text text-transparent font-semibold">
                  Today, you taught ours.
                </span>
              </p>
              ...
              <div className="flex flex-wrap items-center justify-center gap-3">
                ...
                <button
                  onClick={handleShare}
                  className={`inline-flex items-center gap-2 px-5 py-3.5 rounded-xl border font-medium transition-all duration-200 ${
                    isDark
                      ? "border-white/12 text-white/55 hover:border-brand-violet hover:text-brand-violet"
                      : "border-gray-200 text-gray-500 hover:border-brand-violet hover:text-brand-violet"
                  }`}
                >
        ...
        {/* ════════════════ FOOTER CTA STRIP ════════════════ */}
        <Section tinted className="py-16 px-6">
          <div className={`max-w-3xl mx-auto text-center border-t pt-16 ${
            isDark ? "border-white/[0.05]" : "border-gray-100"
          }`}>
            <h3 className={`text-2xl md:text-3xl font-display font-bold mb-4 ${isDark ? "text-white" : "text-gray-900"}`}>
              Ready to stop guessing?
            </h3>
            <p className={`text-base mb-8 ${isDark ? "text-white/35" : "text-gray-400"}`}>
              Join product teams who ship better with AI-powered experimentation.
            </p>
            <Link to="/register" className={ctaPrimary}>
              Start Free Trial →
            </Link>
