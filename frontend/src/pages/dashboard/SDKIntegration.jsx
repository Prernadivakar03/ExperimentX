...
function CopyButton({ text, isDark, disabled }) {
  const [copied, setCopied] = useState(false);
  return (
    <button
      disabled={disabled}
      ...
      className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border transition-colors disabled:opacity-30 disabled:cursor-not-allowed ${
        isDark
          ? "border-white/[0.08] text-white/40 hover:text-white hover:border-white/20"
          : "border-gray-200 text-gray-500 hover:text-gray-900"
      }`}
...
export default function SDKIntegration() {
  ...
  // Real API keys — fetched from the backend, never fabricated.
  const [apiKeys, setApiKeys] = useState([]);
  const [loadingKeys, setLoadingKeys] = useState(true);
  const [creatingKey, setCreatingKey] = useState(false);
  // Only ever populated the moment a key is created — the plaintext key
  // is never retrievable again after this, by design (same as GitHub/Stripe).
  const [justCreatedKey, setJustCreatedKey] = useState(null);
  const [keyError, setKeyError] = useState("");

  const orgId = localStorage.getItem("experimentx_active_org_id");
  ...
  useEffect(() => {
    ...
    api.get(`/organizations/${orgId}/api-keys/`)
      ...
      .catch(() => setKeyError("Couldn't load your API keys."))
      .finally(() => setLoadingKeys(false));
  }, [orgId]);

  const activeKeys = apiKeys.filter((k) => !k.revoked_at);
  const hasAnyKey = activeKeys.length > 0 || !!justCreatedKey;

  const handleGenerateKey = async () => {
    if (!orgId) {
      setKeyError("No active organization found — try reloading.");
      return;
    }
    setCreatingKey(true);
    setKeyError("");
    try {
      const r = await api.post(`/organizations/${orgId}/api-keys/`, {
        name: "SDK Integration key",
      });
      setJustCreatedKey(r.data);
      setApiKeys((prev) => [
        { id: r.data.id, name: r.data.name, key_prefix: r.data.key_prefix, created_at: r.data.created_at },
        ...prev,
      ]);
    } catch (e) {
      setKeyError(e.response?.data?.detail || "Couldn't create a key. Only org admins can generate SDK keys.");
    } finally {
      setCreatingKey(false);
    }
  };

  // What goes into the code snippets below:
  //  - the real plaintext key, if one was just generated this session
  //  - otherwise a clearly-labeled placeholder built from the real prefix
  //    of an existing key (never a fabricated key that looks functional)
  //  - otherwise a generic placeholder telling them to generate one first
  const apiKey = justCreatedKey?.full_key
    || (activeKeys[0] ? `${activeKeys[0].key_prefix}...YOUR_SECRET_KEY` : "YOUR_API_KEY");
  const expId = selectedExp?.id || "your-experiment-id";
  const code = CODE[framework]?.(apiKey, expId) || "";
  ...
  const steps = [
    ...
    {
      n: "02", title: "Generate your API key", desc: "Create a real org-scoped key below to authenticate SDK calls.",
      done: hasAnyKey,
    },
    {
      n: "03", title: "Install the SDK", desc: "npm install experimentx-sdk (browser JavaScript SDK).",
      done: false,
    },
    {
      n: "04", title: "Assign variants", desc: "Call getVariant() with your experiment ID to get A or B for each visitor.",
  ...
  const flowSteps = [
    { icon: User, label: "Visitor arrives", sub: "Any page in your application" },
    { icon: GitBranch, label: "SDK requests variant", sub: "getVariant(experimentId)" },
    { icon: Target, label: "Variant assigned", sub: "Deterministic assignment" },
    { icon: BarChart3, label: "Events tracked", sub: "Clicks, signups, purchases" },
    { icon: BadgeCheck, label: "Results calculated", sub: "Statistical significance" },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`${cardCls} p-5`}>
        ...
        <div className="space-y-3">
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <div>
                <span className={`text-xs font-medium ${isDark ? "text-white/50" : "text-gray-600"}`}>API Key</span>
                <span className={`text-xs ml-2 ${isDark ? "text-white/20" : "text-gray-400"}`}>— Use this to initialize the SDK</span>
              </div>
              {hasAnyKey && (
                <CopyButton text={justCreatedKey?.full_key || activeKeys[0]?.key_prefix || ""} isDark={isDark} />
              )}
            </div>

            {loadingKeys ? (
              <div className={`px-3 py-2.5 rounded-xl text-xs ${isDark ? "bg-white/[0.03] text-white/30 border border-white/[0.06]" : "bg-gray-50 text-gray-400 border border-gray-200"}`}>
                Loading your keys…
              </div>
            ) : justCreatedKey ? (
              <div>
                <div className={`px-3 py-2.5 rounded-xl font-mono text-xs break-all border ${
                  isDark ? "bg-emerald-500/5 text-emerald-300 border-emerald-500/20" : "bg-emerald-50 text-emerald-700 border-emerald-200"
                }`}>
                  {justCreatedKey.full_key}
                </div>
                <p className={`flex items-center gap-1.5 text-[11px] mt-1.5 ${isDark ? "text-amber-400/80" : "text-amber-600"}`}>
                  <AlertTriangle size={12} /> Copy this now — it won't be shown again.
                </p>
              </div>
            ) : activeKeys.length > 0 ? (
              <div>
                <div className={`px-3 py-2.5 rounded-xl font-mono text-xs break-all ${
                  ...
                  {activeKeys[0].key_prefix}…
                </div>
                <p className={`text-[11px] mt-1.5 ${isDark ? "text-white/25" : "text-gray-400"}`}>
                  Full key was only shown once, at creation.{" "}
                  <button
                    onClick={handleGenerateKey}
                    disabled={creatingKey}
                    className={`underline underline-offset-2 ${isDark ? "text-white/40 hover:text-white" : "text-gray-500 hover:text-gray-800"}`}
                  >
                    Generate a new key
                  </button>{" "}
              ...
              <div>
                ...
                {keyError && (
                  <p className="text-[11px] mt-1.5 text-red-400">{keyError}</p>
                )}
              </div>
            )}
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <div>
                <span className={`text-xs font-medium ${isDark ? "text-white/50" : "text-gray-600"}`}>Experiment ID</span>
                <span className={`text-xs ml-2 ${isDark ? "text-white/20" : "text-gray-400"}`}>— Pass this to getVariant() and track()</span>
              </div>
              <CopyButton text={expId} isDark={isDark} />
            </div>
            <div className={`px-3 py-2.5 rounded-xl font-mono text-xs break-all ${
              isDark ? "bg-white/[0.03] text-white/50 border border-white/[0.06]" : "bg-gray-50 text-gray-500 border border-gray-200"
            }`}>
      ...
      <div className={`${cardCls} overflow-hidden`}>
        ...
        {framework !== "vanilla" && (
          <div className={`px-5 py-2 text-[11px] border-b ${isDark ? "border-white/[0.05] text-amber-400/70 bg-amber-500/5" : "border-gray-100 text-amber-600 bg-amber-50"}`}>
            Only the vanilla JavaScript SDK is published today — this snippet shows the planned {FRAMEWORKS.find(f => f.id === framework)?.label} API and won't run yet.
          </div>
        )}
          ...
          {flowSteps.map((step, i) => {
            ...
            return (
              <div key={step.label} className="flex md:flex-col items-center gap-3 md:gap-1 flex-1">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${isDark ? "bg-white/[0.05]" : "bg-gray-50"}`}>
                  <Icon size={20} className={isDark ? "text-white" : "text-gray-700"} />
                </div>
                <div className="md:text-center">
                ...
                {i < flowSteps.length - 1 && (
                  <ArrowRight size={16} className={`hidden md:block ${isDark ? "text-white/20" : "text-gray-400"}`} />
                )}
              </div>
            );
          })}
