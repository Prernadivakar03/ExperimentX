...
import { getExperimentSettings, updateExperimentSettings } from "../../api/organizationSettings";
...
// 3. Experiments
function ExperimentsTab({ isDark, orgId }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await getExperimentSettings(orgId);
        if (cancelled) return;
        const s = res.data;
        setDefaultTraffic(s.default_traffic_pct);
        setConfidenceLevel(s.default_confidence_level);
        setExperimentType(s.default_experiment_type);
        setDefaultDuration(s.default_duration_days);
        setAutoStop(s.auto_stop);
        setRequireMinSample(s.require_min_sample);
        setStatMethod(s.stat_method);
        setSignificance(s.significance_level);
        setCorrection(s.multiple_testing_correction);
      } catch (err) {
        console.error("Failed to load experiment settings:", err);
        toast.error("Could not load experiment settings — showing defaults");
      } finally {
  ...
  const handleSaveDefaults = async () => {
    ...
    try {
      await updateExperimentSettings(orgId, {
        default_traffic_pct: defaultTraffic,
        default_confidence_level: confidenceLevel,
        default_experiment_type: experimentType,
        default_duration_days: defaultDuration,
        auto_stop: autoStop,
        require_min_sample: requireMinSample,
      });
      toast.success("Experiment defaults saved");
    } catch (err) {
      toast.error(err?.response?.data?.detail || "Failed to save experiment defaults");
    } finally {
  ...
  const handleSaveStats = async () => {
    ...
    try {
      ...
      toast.error(err?.response?.data?.detail || "Failed to save statistical settings");
    } finally {
    ...
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="py-2">
        ...
        {activeTab === "experiments" && (
          <ExperimentsTab isDark={isDark} orgId={orgId} />
        )}
        {activeTab === "developer" && (
          <DeveloperTab
            isDark={isDark}
