    ...
    {experiment.allocation_mode === "bandit" && (
      ...
      {stats ? (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <StatCard
            label="P-value"
            value={stats.statistics?.p_value != null ? (stats.statistics.p_value < 0.000001 ? "<0.000001" : stats.statistics.p_value) : "—"}
            sub={stats.statistics?.is_significant ? "✓ Significant" : "Keep running"}
            isDark={isDark}
            color={stats.statistics?.is_significant ? "text-emerald-500" : isDark ? "text-white" : "text-gray-900"}
          />
          ...
          {stats.statistics && !stats.statistics.error && (
            ...
            >
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
                {[
                  { label: "Z-score", value: stats.statistics.z_score },
                  { label: "P-value", value: stats.statistics.p_value < 0.000001 ? "<0.000001" : stats.statistics.p_value },
                  {
                    label: "Relative lift",
                    value: stats.statistics.relative_lift_pct != null
                      ? `${stats.statistics.relative_lift_pct >= 0 ? "+" : ""}${stats.statistics.relative_lift_pct}%`
                      : "—",
                  },
                  {
                    label: "Winner",
              ...
              <div>
                <div className="flex justify-between text-xs mb-2">
                  <span className={isDark ? "text-white/30" : "text-gray-400"}>95% CI for lift</span>
                  <span className={`font-medium ${isDark ? "text-white/70" : "text-gray-700"}`}>
                    {stats.statistics.absolute_lift_ci_95
                      ? `[${stats.statistics.absolute_lift_ci_95.lower_pct_points >= 0 ? "+" : ""}${stats.statistics.absolute_lift_ci_95.lower_pct_points}, ${stats.statistics.absolute_lift_ci_95.upper_pct_points >= 0 ? "+" : ""}${stats.statistics.absolute_lift_ci_95.upper_pct_points}] pp`
                      : "—"}
                  </span>
                </div>
                <div className="flex justify-between text-xs mb-2">
                  <span className={isDark ? "text-white/30" : "text-gray-400"}>Power at current sample size</span>
                  <span className={`font-medium ${
                    stats.statistics.achieved_power >= 0.8 ? "text-emerald-500" : isDark ? "text-white/50" : "text-gray-600"
                  }`}>
                    {stats.statistics.achieved_power != null ? `${Math.round(stats.statistics.achieved_power * 100)}%` : "—"}
                  </span>
                </div>
                <div className={`h-2.5 rounded-full ${isDark ? "bg-white/[0.06]" : "bg-gray-100"}`}>
                  <motion.div
                ...
                <p className={`text-xs mt-2 ${isDark ? "text-white/20" : "text-gray-400"}`}>
                  {stats.statistics.is_significant
                    ? `Variant ${stats.statistics.winner} is statistically significant at p < ${stats.statistics.alpha ?? 0.05}. Safe to consider a winner.`
                    : stats.statistics.achieved_power != null && stats.statistics.achieved_power < 0.8
                    ? "Not significant yet, and underpowered at the current sample size — a real effect may still be there but undetectable yet. Keep collecting data."
                    : "Collect more data before declaring a winner."}
                </p>
              </div>
            </motion.div>
          )}
