...
export default function Overview({ onNavigate }) {
  ...
  const [trends, setTrends] = useState({ visitors: 0, conversions: 0, conversion_rate: 0 });
  const [loading, setLoading] = useState(true);
  // True only when the /experiments/ call itself failed — kept separate
  // from "genuinely zero experiments" so a transient network/API error
  // can never masquerade as an empty account and send you to Onboarding.
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      api.get("/analytics/"),
      api.get("/experiments/"),
      api.get("/analytics/overview/timeseries", { params: { days: 7 } }),
    ]).then(([statsRes, expRes, tsRes]) => {
      if (cancelled) return;

      if (statsRes.status === "fulfilled") {
        setStats(statsRes.value.data);
      } else {
        console.error("Failed to load analytics summary:", statsRes.reason);
      }

      if (expRes.status === "fulfilled") {
        setExperiments(expRes.value.data);
      } else {
        console.error("Failed to load experiments:", expRes.reason);
        setLoadError(true);
      }

      if (tsRes.status === "fulfilled") {
        setTimeseries(tsRes.value.data?.series || []);
        setTrends(tsRes.value.data?.trends || { visitors: 0, conversions: 0, conversion_rate: 0 });
      } else {
        // Timeseries only feeds the chart + trend badges — fail quietly
        // and keep the safe defaults rather than breaking the page.
        console.error("Failed to load overview timeseries:", tsRes.reason);
      }
  ...
  const sparkA = timeseries.map((d) => ({ v: d.visitors }));
  const sparkB = timeseries.map((d) => ({ v: d.conversions }));

  // Only show onboarding when we actually know the account has zero
  // experiments — never as a side effect of a failed request.
  if (!loading && !loadError && experiments.length === 0) {
    ...
    <div className="space-y-6">
      ...
      {loadError && (
        <div className={`flex items-center justify-between gap-3 px-4 py-3 rounded-xl border text-sm ${
          isDark ? "bg-red-400/5 border-red-400/20 text-red-300" : "bg-red-50 border-red-200 text-red-700"
        }`}>
          <span>Couldn't load your experiments — showing what we have.</span>
          <button onClick={() => window.location.reload()} className="underline font-medium flex-shrink-0">
            Retry
          </button>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        <KPICard label="Total Visitors" value={stats?.total_visitors?.toLocaleString() || "0"}
          trend={trends.visitors} sub="vs previous 7 days" accent="#6C5CE7" delay={0}
          spark={sparkA} />
        <KPICard label="Conversions" value={stats?.total_conversions?.toLocaleString() || "0"}
          trend={trends.conversions} sub={`${stats?.total_experiments || 0} experiments`} accent="#4F8CFF" delay={0.05}
          spark={sparkB} />
        <KPICard label="Avg. Conv. Rate"
          value={stats?.total_visitors > 0
            ? `${((stats.total_conversions / stats.total_visitors) * 100).toFixed(2)}%`
            : "0%"}
          trend={trends.conversion_rate} sub="across all experiments" accent="#10B981" delay={0.1} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        >
          ...
          {timeseries.length === 0 ? (
            <div className="h-[200px] flex items-center justify-center">
              <p className={`text-xs ${isDark ? "text-white/25" : "text-gray-400"}`}>No traffic data yet</p>
            </div>
          ) : (
        ...
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => onNavigate?.("experiments")} className="text-xs text-brand-ember hover:underline">
              View all
            </button>
          </div>
          {running.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-32">
              ...
              <button onClick={() => onNavigate?.("experiments")}
                className="mt-2 text-xs text-brand-ember hover:underline">Create one →</button>
            </div>
          ) : (
            <div className="space-y-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <div className={`flex-1 h-1 rounded-full ${isDark ? "bg-white/10" : "bg-gray-100"}`}>
                        <div className="h-full rounded-full bg-brand-ember" style={{ width: "60%" }} />
                      </div>
                    ...
                    <td className="px-5 py-3.5 hidden sm:table-cell">
                      <span className={`inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full ${
                        e.status === "running" ? "bg-emerald-500/10 text-emerald-500"
                          : e.status === "completed" ? "bg-brand-ember/10 text-brand-ember"
