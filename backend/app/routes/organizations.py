...
from app.schemas.organization_schema import (
    ...
    ExperimentSettings, ExperimentSettingsUpdate,
)
...
# Org-wide defaults shown on Settings > Experiments. Stored as one JSON blob
# on the organization row so new default fields don't need a migration each
# time — see app/schemas/organization_schema.py::ExperimentSettings for the
# full shape and defaults applied when a field is missing.

@router.get("/{org_id}/settings", response_model=ExperimentSettings)
def get_experiment_settings(
    ...
    # Merge stored settings over the schema defaults so a fresh org (empty
    # dict) still returns a fully populated, valid response.
    return ExperimentSettings(**{**ExperimentSettings().model_dump(), **(org.settings or {})})


@router.put("/{org_id}/settings", response_model=ExperimentSettings)
def update_experiment_settings(
    org_id: UUID,
    payload: ExperimentSettingsUpdate,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    check_org_access(org_id, current_user, db, MemberRole.admin)
    ...
    current = {**ExperimentSettings().model_dump(), **(org.settings or {})}
    updates = {k: v for k, v in payload.model_dump().items() if v is not None}
    current.update(updates)

    # Sanity-clamp the ranges the frontend inputs allow — belt-and-suspenders,
    # the frontend already validates these but the API shouldn't trust it alone.
    current["default_traffic_pct"] = max(0, min(100, current["default_traffic_pct"]))
    current["default_confidence_level"] = current["default_confidence_level"] if current["default_confidence_level"] in (90, 95, 99) else 95
    current["default_duration_days"] = max(1, current["default_duration_days"])
    current["significance_level"] = max(0.001, min(0.5, current["significance_level"]))

    org.settings = current
    db.commit()
    db.refresh(org)
    return ExperimentSettings(**current)
