...
#     p_a = a["conversions"] / a["visitors"] if a["visitors"] else 0
#     p_b = b["conversions"] / b["visitors"] if b["visitors"] else 0

#     result["winner"] = (
#         b["label"] if p_b > p_a else a["label"]
#     ) if result["is_significant"] else None
...
from app.core.bayesian import bayesian_test
from app.core.sequential import msprt_multi_variant
from app.core.cuped import cuped_two_variant_test
from app.database import get_db
from app.dependencies import get_current_user
from app.models.user import User
from app.models.organization import MemberRole
...
def _cuped_result(experiment_id: UUID, variant_a: Variant, variant_b: Variant, db: Session) -> dict:
    def _visitor_outcomes(variant_id):
        rows = db.query(Visitor.id, Visitor.pre_experiment_covariate).filter(
            Visitor.experiment_id == experiment_id, Visitor.variant_id == variant_id,
            Visitor.pre_experiment_covariate.isnot(None),
        ).all()
        if not rows:
            return [], []
        visitor_ids = [r[0] for r in rows]
        converted_ids = {
            row[0] for row in db.query(Conversion.visitor_id)
            .filter(Conversion.visitor_id.in_(visitor_ids)).distinct().all()
        }
        outcomes = [1.0 if vid in converted_ids else 0.0 for vid, _ in rows]
        covariates = [cov for _, cov in rows]
        return outcomes, covariates

    outcomes_a, covariates_a = _visitor_outcomes(variant_a.id)
    outcomes_b, covariates_b = _visitor_outcomes(variant_b.id)

    if len(outcomes_a) < 30 or len(outcomes_b) < 30:
        return {
            "available": False,
            "reason": f"Needs 30+ visitors with pre-experiment history per variant (have {len(outcomes_a)} / {len(outcomes_b)}).",
        }

    return cuped_two_variant_test(outcomes_a, covariates_a, outcomes_b, covariates_b)
...
def get_analytics(
    ...
    if len(variant_stats) == 2:
        ...
        cuped_result = _cuped_result(experiment_id, variants[0], variants[1], db)
    elif len(variant_stats) > 2:
        stats_result = multi_variant_test(variant_stats)
