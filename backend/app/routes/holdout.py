...
def check_holdout(owner_id: UUID, fingerprint: str, db: Session) -> HoldoutVisitor | None:
    ...
    NOTE: still keyed by owner_id, not organization_id — unchanged
    deliberately, same reasoning as evaluate_flag in flags.py.
    """
...
#         HoldoutGroup.organization_id == org_id, HoldoutGroup.is_active == True,
#     ).first()
#     if existing_active:
#         raise HTTPException(
#             status_code=409,
...
#     group = HoldoutGroup(owner_id=current_user.id, organization_id=org_id, **payload.model_dump())
#     db.add(group)
#     db.commit()
#     db.refresh(group)
#     return group
...
#         ).all()
#     ]
#     return db.query(HoldoutGroup).filter(HoldoutGroup.organization_id.in_(org_ids)).all()
...
def list_holdout_groups(
    ...
    return db.query(HoldoutGroup).filter(HoldoutGroup.organization_id == org_id).all()
...
def update_holdout_group(
    ...
    group = db.query(HoldoutGroup).filter(HoldoutGroup.id == group_id).first()
    if not group:
        raise HTTPException(status_code=404, detail="Holdout group not found")
    check_org_access(group.organization_id, current_user, db, minimum_role=MemberRole.editor)
...
def get_holdout_impact(
    ...
    if not group:
        raise HTTPException(status_code=404, detail="Holdout group not found")
    check_org_access(group.organization_id, current_user, db, minimum_role=MemberRole.viewer)
    ...
    # Baseline = everyone who WAS exposed to an experiment, across the whole org now
    experiment_ids = [
        e.id for e in db.query(Experiment).filter(Experiment.organization_id == group.organization_id).all()
    ]
    ...
    return HoldoutImpactResponse(
        ...
        note=(
            ...
            "everyone who WAS in an experiment, across your whole organization. It's a directional "
