...
def track_event(
    ...
    # Validate the variant belongs to this experiment -- without this, a
    # request could pair a valid experiment_id and visitor_id with a
    # variant_id from a completely different experiment, silently
    # corrupting that variant's analytics.
    variant = db.query(Variant).filter(
        Variant.id == payload.variant_id,
        Variant.experiment_id == payload.experiment_id,
    ).first()
    if not variant:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Variant not found for this experiment",
        )
...
def track_conversion(
    ...
    db: Session = Depends(get_db),
    _rl=Depends(rate_limit_by_ip("30/minute")),
    organization: Organization = Depends(get_org_from_api_key),
):
    ...
    try:
        ...
        # The application-level check above already handles the common
        # case; this only fires on a genuine race — two concurrent
        # requests both passing the check before either committed.
        db.rollback()
        return {"message": "Conversion already recorded for this visitor — ignored duplicate"}
