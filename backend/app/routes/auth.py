...
def logout(request: Request, response: Response, db: Session = Depends(get_db)):
    """Revoke the refresh token and clear its cookie. Frontend must also
...
REFRESH_COOKIE_NAME = "experimentx_refresh_token"
...
def _issue_tokens(user: User, db: Session, response: Response) -> dict:
    ...
    # Refresh token goes ONLY in the httpOnly cookie — never in the JSON
    # body, so it's not reachable from frontend JS at all (not even
    # momentarily, in the fetch/axios response).
